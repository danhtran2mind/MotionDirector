...
def tensor_to_vae_latent(t, vae):
    ...
    print(f"Input pixel_values shape: {t.shape}")
    print(f"Latents shape after VAE encoding: {latents.shape}")
...
def main(
    ...
    def finetune_unet(batch, step, mask_spatial_lora=False, mask_temporal_lora=False):
        ...
        print(f"Latents shape in finetune_unet: {latents.shape}")
        ...
        if mask_spatial_lora:
            ...
            print(f"Noisy latents input shape: {noisy_latents_input.shape}")
            model_pred_spatial = unet(noisy_latents_input, timesteps, encoder_hidden_states=encoder_hidden_states).sample
            print(f"Model pred spatial shape: {model_pred_spatial.shape}")
        ...
        if mask_temporal_lora:
            ...
            print(f"Model pred temporal shape: {model_pred.shape}")
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % checkpointing_steps == 0 and global_step > 0:
                    save_pipe(
                        ...
                        vae,
                        output_dir,
                        lora_managers_spatial[0] if spatial_lora_num == 1 else lora_managers_spatial[step],
    ...
    if accelerator.is_main_process:
        save_pipe(
            ...
            vae,
            output_dir,
            lora_managers_spatial[0] if spatial_lora_num == 1 else lora_managers_spatial[-1],
            lora_manager_temporal,
            unet_lora_modules,
            text_encoder_lora_modules,
            is_checkpoint=False,
