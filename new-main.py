...
def finetune_unet(
    ...
    # Clear cache after forward pass to save VRAM
...
def main(
    ...
    gradient_checkpointing: bool=True,  # Enabled by default for memory savings
    text_encoder_gradient_checkpointing: bool=True,  # Enabled for memory savings
    checkpointing_steps: int=500,
    resume_from_checkpoint: str=None,
    resume_step: int=None,
    mixed_precision: str="fp16",
    use_8bit_adam: bool=True,  # Enabled for memory efficiency
    ...
    cache_latents: bool=True,  # Enabled by default to reduce VAE computation
    cached_latent_dir: str=None,
    use_unet_lora: bool=True,  # Enabled for memory-efficient fine-tuning
    unet_lora_modules: tuple=[],
    text_encoder_lora_modules: tuple=[],
    save_pretrained_model: bool=True,
    lora_rank: int=8,  # Reduced rank for lower memory usage
    lora_path: str='',
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                logger.info(f"Step {global_step}: Memory allocated: {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
                logger.info(f"Step {global_step}: Memory reserved: {torch.cuda.memory_reserved() / 1024**3:.2f} GB")
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % checkpointing_steps == 0 and global_step > 0:
                    ...
                    torch.cuda.empty_cache()  # Clear cache after checkpointing
